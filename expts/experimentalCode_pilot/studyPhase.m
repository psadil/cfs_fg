...
%--------------------------------------------------------------------------
% text
text_start = 'The study phase of the experiment will begin when you press the space bar. REMEMBER: please keep your eyes relaxed and focused on the white square in the center of the screen. AVOID blinking or moving.';
...
for trial = 1:p.nItems
    ...
    %         end
    %     elseif any(p.itemCondition_study(trial) == 1) % if 1: binoc
    % %         leftRight = [1,1]; % call CFS with left/righ eye == image+mondrian,
    %         if p.rightEyeDom
    %             leftRight = [1,2]; % non-dom eye (left) receives image+white square (while dom (right) receives image+mondrian)
    %         else
    %             leftRight = [2,1]; % non-dom eye (right) receives image+white square (while dom (left) receives image+mondrian)
    %         end
    %     elseif any(p.itemCondition_study(trial) == 2)% if 2: CFS
    %         if p.rightEyeDom
    ...
    if p.itemCondition_study(trial) == 0 % 1.5 seconds
        texAlpha = p.texAlpha.secs_one_five;
    elseif p.itemCondition_study(trial) == 1 % 2 seconds
        texAlpha = p.texAlpha.secs_two;
    elseif p.itemCondition_study(trial) == 2 % 2.5 seconds
        texAlpha = p.texAlpha.secs_two_five;
    elseif p.itemCondition_study(trial) == 3
        texAlpha = p.texAlpha.secs_three;
    end
