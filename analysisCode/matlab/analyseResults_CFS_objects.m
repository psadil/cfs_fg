...
for subNum = start:numSubjects,
    ...
    %----------------------------------------------------------------------
    % load the data
    fName = [pwd '\Subject Data\currentVersion\pilotData\raw\Subject' num2str(subNum) 'PDP_CFS_objects1.mat'];
    fprintf('\nSUBJECT %d:', subNum);
    load(fName)
    ...
    for block = 1:2
        ...
        for t = 1:p.nItems/2,
            trial = trial+1;
            % need to make in correct space (either item or trial)
            ...
            % find which answer was given to the item during study
%             studyTrial = find(p.studyOrder==item);
            
            % record that answer
            results.test.studyResp(trial) = str2double(p.responses.study(p.studyOrder==item,1));
            ...
            if isempty(levDist)
                fprintf('\nTrial #%d (Object%d). studied, but no answer given', trial, p.testOrder(trial));
                results.test.recallCorrect(trial) = 0;
                continue
            elseif length(response) < 2;
                fprintf('\nTrial #%d (Object%d). no response given', trial, p.testOrder(trial));
                results.test.recallCorrect(trial) = 0;
                continue
            else
            ...
            %                 fprintf('\nSTUDY RESPONSE: %s \nTEST: %s \n', closestName, response);
            %             else
            fprintf('\nTrial #%d (Object%d). Studied: %d.', trial, p.testOrder(trial),results.test.studied(trial));
            fprintf('\nPOSSIBLE RESPONSE: %s \nTEST: %s \n', closestName, response);
            %             end
