function analyse_CFS_objects_outPut_pilot( firstSub, lastSub )
...
for subNum = firstSub:lastSub
    ...
    twoPfive = p.twoPfive;
    three = p.three;
    for item = 1:p.nItems*3
       out.onePfive(item) = onePfive(item);
       out.two(item) = two(item);
       out.twoPfive(item) = twoPfive(item);
       three = results.study.three(item);
    end
    
    out.named = results.study.named;
    out.notNamed = results.study.notNamed;
    
    %----------------------------------------------------------------------
    % variables to divide the data by study response (along PAS)
    % 0 => no experience
    % 1 => brief glimpse
    % 2 => almost clear
    % 3 => clear experience
    
    out.zero = p.responses.study==0;
    out.one = p.responses.study==1;
    out.two = p.responses.study==2;
    out.three = p.responses.study==3;
    out.noStudyResp = isnan(p.responses.study);
    % those should sum (all of them combined) to 360 for each participant
