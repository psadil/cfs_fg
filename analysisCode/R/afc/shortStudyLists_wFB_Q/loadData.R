...
nS$whichItem <- length(unique(data$whichItem))

data <- mutate(data, pas1 = plyr::mapvalues(pas1, from=c(-1,NA), c('NR','fNS'))) %>%
  mutate(., pas2 = plyr::mapvalues(pas2, from=c(-1,NA), c('NR','fNS')))


data[,c('subject','condition','whichItem','studyResp','studyResp2','pas1','pas2','list')] <-
  lapply(data[,c('subject','condition','whichItem','studyResp','studyResp2','pas1','pas2','list')],
         FUN=factor)
