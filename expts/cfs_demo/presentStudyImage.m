...
%   called by demoPhase during each trial

% 11feb2016 ps - writing demo for prospective students
...
%         Screen('DrawTexture',p.window,p.whiteTex,[],p.whiteRect,[],[],texAlpha(tick)/100);
    ...
    Screen('DrawingFinished',p.window);
        [VBLTimestamp(tick+1), StimulusOnsetTime(tick+1), FlipTimestamp(tick+1), ~, ~] = Screen('Flip', p.window,...
            FlipTimestamp(tick)+(1/p.hzRate)-(1/(p.RefreshRate*2)));
%     vbl(tick+1) = Screen('Flip', p.window, vbl(tick) + (1/p.hzRate) - (1/p.RefreshRate*2));
