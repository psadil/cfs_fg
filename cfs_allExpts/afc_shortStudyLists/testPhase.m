...
currentListInd = p.ind.testList(list):(p.ind.test(list) + p.nItems.studyList-1);
...
for testItem = 1:p.nItems.studyList
    ...
    if p.test_leftRight(trial) == 1
        p = afcResp(p,trialsTest(item).p1, trialsTest(item).p2);
    else
        p = afcResp(p,trialsTest(item).p2, trialsTest(item).p1);
    end
