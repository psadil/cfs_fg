...
trial = 0;
for block = 1:2
    if p.blockInEx(1) == block
        %% Begin instructions for test Inclusion
        inclusion = 1;
    else
        inclusion = 0;
    end
    
    p = testInstructions(p, inclusion);
    ...
    for t = 1:(p.nItems/2)
        ...
        p = recallResp(p, trialsTest(trial).tex, trial, inclusion);
    ...
    p.timing.endTestPhase(block) = GetSecs;
    p.dur.testPhase(block) = p.timing.endTestPhase(block)-p.timing.startTestPhase(block);
end % End of all possible testing
