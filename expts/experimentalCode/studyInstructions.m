...
text_first2 = 'On some of the trials, you will also notice an image behind the colored squares.';
text_first3 = 'You will see an example of the squares on the next slide.';

text_second1 = 'After every trial, you will be asked what you saw.';
...
text_third2 = 'If you could CLEARLY SEE something besides the squares (like a tree), please press 3.';
text_third3 = 'If you saw something that was ALMOST CLEAR, press 2';
text_third4 = 'If you only caugt a BRIEF GLIMPSE of something besides the squares, but aren''t sure what it was, press 1';
...
tCenterSecond1 = [p.xCenter-RectWidth(Screen('TextBounds', p.window, text_second1))/2  p.yCenter-160];
...
presentStudyImage(p,1,p.texture_ITI,[0,0])
...
while 1
    ...
    Screen('SelectStereoDrawBuffer',p.window,(0));
    Screen(p.window,'DrawTexture', p.texture_ITI);
    Screen('DrawText', p.window, text_second1, tCenterSecond1(1), tCenterSecond1(2), p.textColor);
...
filename_whole=strcat(pwd, '\stims\object121_whole_noBackground.png'); %Grab the tree
[image_whole, mapCol, transperancy] = imread(filename_whole, 'background', 'none');
image_whole(:,:,4)=transperancy;
texture_wholeObject = Screen('MakeTexture',p.window, image_whole);

% present tree image to both eyes
if p.rightEyeDom
    presentStudyImage(p,1,texture_wholeObject,[1,2])
else
    presentStudyImage(p,1,texture_wholeObject,[2,1])
end
